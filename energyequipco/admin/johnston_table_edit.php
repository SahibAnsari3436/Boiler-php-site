...
$page_title = isset($page_titles[$current_file]) ? $page_titles[$current_file] : 'Add johnston Table Entry';
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = mysqli_prepare($con, "SELECT * FROM johnston_table WHERE item_number=?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['old_item_number'])) {
        ...
        $stmt = mysqli_prepare($con, "UPDATE johnston_table SET item_number=?, year=?, horsepower=?, manufacturer=?, pressure=?, steam_or_hot_water=?, burner=?, fuel=? WHERE item_number=?");
        ...
        // New entry
        $stmt = mysqli_prepare($con, "INSERT INTO johnston_table (item_number, year, horsepower, manufacturer, pressure, steam_or_hot_water, burner, fuel) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
