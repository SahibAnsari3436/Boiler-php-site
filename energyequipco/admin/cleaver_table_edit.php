...
$page_title = isset($page_titles[$current_file]) ? $page_titles[$current_file] : 'Add Cleaver Table Entry';
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = mysqli_prepare($con, "SELECT * FROM cleaver_table WHERE item_number=?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['old_item_number'])) {
        ...
        $stmt = mysqli_prepare($con, "UPDATE cleaver_table SET item_number=?, year=?, horsepower=?, manufacturer=?, pressure=?, steam_or_hot_water=?, burner=?, fuel=? WHERE item_number=?");
        ...
        // New entry
        $stmt = mysqli_prepare($con, "INSERT INTO cleaver_table (item_number, year, horsepower, manufacturer, pressure, steam_or_hot_water, burner, fuel) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sissssss", $item_number, $year, $horsepower, $manufacturer, $pressure, $steam_or_hot_water, $burner, $fuel);
    }

    mysqli_stmt_execute($stmt);
    header("Location: cleaver_table.php");
    exit;
}
?>
