...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $con->prepare("SELECT * FROM boiler_details WHERE id = ?");
...
if (isset($_POST['submit'])) {
    ...
    if ($id) {
        // Update
        $stmt = $con->prepare("UPDATE boiler_details SET image = ?, description = ? WHERE id = ?");
        $stmt->bind_param("ssi", $image, $description, $id);
    } else {
        // Insert
        $stmt = $con->prepare("INSERT INTO boiler_details (image, description) VALUES (?, ?)");
