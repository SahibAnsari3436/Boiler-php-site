...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $con->prepare("SELECT * FROM other_boilers WHERE id = ?");
...
if (isset($_POST['submit'])) {
    ...
    if ($id) {
        // Update
        $stmt = $con->prepare("UPDATE other_boilers SET image = ?, description = ? WHERE id = ?");
        $stmt->bind_param("ssi", $image, $description, $id);
    } else {
        // Insert
        $stmt = $con->prepare("INSERT INTO other_boilers (image, description) VALUES (?, ?)");
