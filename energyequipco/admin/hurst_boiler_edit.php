...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $con->prepare("SELECT * FROM hurst_boiler WHERE id = ?");
...
if (isset($_POST['submit'])) {
    ...
    if ($id) {
        // Update
        $stmt = $con->prepare("UPDATE hurst_boiler SET image = ?, description = ? WHERE id = ?");
        $stmt->bind_param("ssi", $image, $description, $id);
    } else {
        // Insert
        $stmt = $con->prepare("INSERT INTO hurst_boiler (image, description) VALUES (?, ?)");
