<?php
/*
Template Name: Industry - Industry & Factory HTML Template
...
if(isset($_POST)){
	 ...
	 */
     ...
     $email = trim(strip_tags($_POST['icEmail']));
	if(!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL) === false){
        ...
        if ($httpCode == 200) {
			$icRes['status'] = 1;
            $icRes['msg'] = 'You have successfully subscribed.';
        } else {
            switch ($httpCode) {
                case 214:
        ...
        $icRes['msg'] = 'Please enter valid email address.';
    }
